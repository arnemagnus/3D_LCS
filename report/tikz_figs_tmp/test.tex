...
%    % Place the set of initial points
    ...
    \path[draw,stroke=black!65,->,thick,dashdotted ] (3) to ($(3) + 5*({-0.866},{0.5},0)$) coordinate (rhoend);
    ...
    \pic[densely dashed,draw,stroke=grey!80,angle radius=39] {angle = bt--3--tp};



    % Label ancestor point
    \coordinate[below left = 0pt and 30pt of 3] (plbl);
    \node at (plbl) {$\mathcal{M}_{i,j}$};
    \draw[stroke=black!80] ($(plbl)!0.4!(3)$) to[out = 20, in = 160]
        ($(plbl)!0.85!(3)$);


    % Define aim point
    \coordinate (aim) at ($(3) + 1.2*({-0.866},{0.5},0) + (0,0,0.75)$);

    % Define support point in semicircle, to indicate radius
    \coordinate (supp) at ($(3) + 1.57*({-0.866},{0.5},0)$);
    % Draw semicircle to indicate angle offset of aim point
    \pic[draw,stroke=gray!80,angle eccentricity = 1.7,angle radius=10] {angle =
    supp--3--aim};
    % Add text node to explain the angle
    \node[below right = 10pt and -2.5pt of 3] (mrk) {$\alpha_{i,j}$};
    % Add anchor for line from node to within angle
    \coordinate (inang) at ($(3) + 0.25*({-0.866},{0.5},0) + (0,0,0.25)$);
    \path[draw,stroke=black!80,thin] ($(mrk)!0.1!(inang)$) to [out = 60, in = -180+60] ($(mrk)!0.8!(inang)$);
    % Draw vector from start point to aim point
    \draw[stroke=black!65,thin,dashed] (3) -- (aim);
    % Redraw start point
    \draw[fill=gray!10,stroke=black!65] (3) circle (\size pt);
    % Draw aim point
