...
%    \draw[draw=gray!70,fill=gray!10] ($(doomed8)+({0.15},-{0.85},0)$) circle (\size pt) coordinate (doomed9);
    ...
    \node[below right = -2.5pt and -2.5pt of doomed6,color=gray!80] {\footnotesize $\mathcal{M}_{i,j+k-2}$};
    \node[below right = -2.5pt and -2.5pt of doomed7,color=gray!80] {\footnotesize $\mathcal{M}_{i,j+k-1}$};
...
%
    % Draw bracket
    \draw[decorate,decoration={brace,amplitude=2.5pt,raise=17.5pt},yshift=0pt] (i2) -- (i3) node [midway,below left = 35pt and -45pt,rotate=-18] {\footnotesize $\Delta_{\min} < \norm{\vct{x}_{i,j+k}-\vct{x}_{i,j}} < \Delta_{\max}$};
