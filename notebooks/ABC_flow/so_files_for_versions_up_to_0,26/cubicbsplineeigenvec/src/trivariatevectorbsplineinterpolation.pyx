...
cdef class Dp87BSpline:
    ...
    Dp87BSpline.__init__(atol, rtol)
    Dp87BSpline.set_aim_assister(direction_generator)
    Dp87BSpline.unset_aim_assister()
    ...
    def set_aim_assister(self, SplineAimAssister direction_generator):
        ...
        Sets the aim assister. Must be called *before* Dp87BSpline.__call__.

        param: direction_generator -- SplineAimAssister instance, initialized
                                      with a BSplineEigenvectorInterpolator
                                      object as well as a target
    ...
    def __call__(self, double t, double[::1] pos, double h):
        ...
        if not self.initialized:
            raise RuntimeError('Dormand-Prince 8(7) linear solver not'\
                    ' initialized with a BSplineAimAssister instance!')
        dcopy(3,pos,1,pos_i,1)
        self._ev_(&t,pos_i,&h)
        return t, np.copy(self._pos_i_), h
