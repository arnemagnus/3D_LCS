...
def cy_normalize(double[::1] v):
    ...
    ----------
    v : (3,) array_like
        Three-component (C-contiguous) NumPy array, containing the vector

    Returns
    -------
    vec : (3,) ndarray
         Normalized vector, as a three-component (C-contiguous) NumPy array.
...
def cy_parallel_component(double[::1] u, double[::1] v):
    ...
    ----------
    u : (3,) array_like
        Three-component (C-contiguous) NumPy array, containing the
        components of u
    v : (3,) array_like
        Three-component (C-contiguous) NumPy array, containing the
        components of v
...
def cy_dot(double[::1] u, double[::1] v):
    ...
    -------
    val : double
         Double-precision representation of the dot product between u and v
...
def cy_compute_pos_aim(double[::1] prev_pos, double dist, double[::1] prev_prev_vec, double[::1] tan_vec, double ang_offset):
    ...
    dist : double
        The (Euclidean) distance from prev_pos, at which a
        new point is sought
    prev_prev_vec : (3, array_like)
        Three-component (C-contiguous) NumPy array,
        containing the components of a vector which defines
        an approximately radially outwards direction from prev_pos
    tan_vec : (3,) array_like
        Three-component (C-contiguous) NumPy array,
    ...
    ang_offset : double
        Double-precision float, giving the angular offset
...
def cy_norm2(double[::1] u):
    ...
    ----------
    u : (n,) array_like
        n-component (C-contiguous) NumPy array, containing the
        components of u

    Returns
    -------
    nrm : double
        2-norm of u, as a double-precision float.
...
def cy_min(double a, double b):
    ...
    --------
    r : double
        Double-precision float, the smallest number of a and b.
...
def cy_max(double a, double b):
    ...
    --------
    r : double
        Double-precision float, the largest number of a and b.
